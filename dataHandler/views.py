...
def get_ent_data(request):
    ...
    if myuser.type == "企业用户":
        ...
        data = EnterpriseInfo.objects.filter(needpre=0)
...
def get_ent_data_all(request):
    ...
    if myuser.type == "企业用户":
        ...
        data = EnterpriseInfo.objects.all()
    dataCount = data.count()
    pageIndex = request.POST.get('pageIndex')
    pageSize = request.POST.get('pageSize')
...
#用于向政府用户返回数据查询页面的函数
@login_required
def get_ent_research_gov(request):
    return render(request,"data_handler/ent_data_search.html")

#用于向政府用户数据查询界面返回数据的函数
@login_required
def get_ent_data_gov(request):
    user = request.user
    myuser = Myuser.objects.get(u=user)
    data = EnterpriseInfo.objects.filter(mu=myuser,needpre=0)
    dataCount = data.count()
...
#用于向政府用户数据查询界面返回所有数据的函数
@login_required
def get_gov_data_all(request):
    # user = request.user
    # myuser = Myuser.objects.get(u=user)
    # data = EnterpriseInfo.objects.filter(mu=myuser)
    dataCount = data.count()
